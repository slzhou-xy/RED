...
class TrajSim(nn.Module):
    ...
    def forward(self, node_feature, edge_index, enc_data, lambda2=0.5):
        traj_x, temporal_x, user_id_x, mask, end_idx, temporal_mat_x, dis_mat_x, highway = enc_data
        spe = self.pretraining_model.spe(node_feature, edge_index)
        ce = self.pretraining_model.ce(highway)
